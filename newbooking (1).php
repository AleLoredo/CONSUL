                                    ...
                                    <div class="card p-2">
                                       ...
                                       <!-- Inicia repetidor de turnos -->
                                        <?php foreach ($dates as $date): 
                                            ...
                                            $dayOfWeek = date('N', strtotime($currentDate)); // 1 (lunes) hasta 7 (domingo)
                                            
                                            // Obtener configuración de horarios para este día de la semana
                                            $stmt = $pdo->prepare("SELECT isworkday, firstbooking, lastbooking FROM bizzhours WHERE id = ?");
                                            $stmt->execute([$dayOfWeek]);
                                            $bizConfig = $stmt->fetch(PDO::FETCH_ASSOC);
                                            
                                            // Si es un día laboral (isworkday = 1), mostrar el formulario
                                            if ($bizConfig && $bizConfig['isworkday'] == 1):
                                                $firstTime = $bizConfig['firstbooking'];
                                                $lastTime = $bizConfig['lastbooking'];
                                        ?>
                                            <form action="newbooking2.php?id=<?php echo $contact_id ?>" method="POST" class="d-flex align-items-center border-bottom p-2">
                                                ...
                                                <select name="selected_time" class="form-select me-2" style="width: auto;" required>
                                                    ...
                                                    foreach ($allTimes as $time): 
                                                        ...
                                                        // Verificar si el horario está dentro del rango permitido para este día
                                                        if ($timeValue >= $firstTime && $timeValue <= $lastTime) {
                                        ...
                                        <?php 
                                            endif; // Fin del if para días laborales
                                        endforeach; 
                                        ?>
